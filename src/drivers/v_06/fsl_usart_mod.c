...
callback_protocol_functions_t* g_ptr_callback_protocol_functions;
...
status_t USART_TransferSendNonBlocking(USART_Type *base, usart_handle_t *handle, usart_transfer_t *xfer)
{
    ...
    else
    {
    	...
    	handle->ptrBuffer 		= xfer->data; 			            // ptr_buffer = (uint8_t* ) p_ptrBuffer;
    	handle->BufferSize   	= xfer->dataSize; 	                // g_buffer_size = SDK_USART_DEFAULT_BUFFER_SIZE;
    	handle->PacketLength 	= 0;								// g_packet_length = 0;
    	handle->DataCounter  	= 0;								// tx_byte_counter = 0;
    	handle->Status			= SDK_USART_TX_STATUS__RUNNING;		// status = SDK_USART_TX_STATUS__RUNNING;
                                          ...
                                          size_t *receivedBytes)
{
    ...
    else
    {
		...
		handle->ptrBuffer 		= xfer->data; 						// ptr_buffer = p_ptrBuffer;
		handle->BufferSize   	= xfer->dataSize;				 	// g_buffer_size = p_size_buffer;
		handle->PacketLength 	= 0;								// g_packet_length = 0;
		handle->DataCounter  	= 0;								// rx_byte_counter = 0;
		handle->Status			= SDK_USART_RX_STATUS__RUNNING;		// status = SDK_USART_RX_STATUS__RUNNING;
...
void USART_TransferHandleIRQ(USART_Type *base, usart_handle_t *handle)
{
    ...
    while (
        ( (g_callback_protocol_functions.rx_status_get__running()) && ((base->FIFOSTAT & USART_FIFOSTAT_RXNOTEMPTY_MASK) != 0U) ) ||
        ( (g_callback_protocol_functions.tx_status_get__running()) && ((base->FIFOSTAT & USART_FIFOSTAT_TXNOTFULL_MASK)  != 0U) )
    ) {
        /* Receive data */
        if ( (base->FIFOSTAT & USART_FIFOSTAT_RXNOTEMPTY_MASK) != 0U ) {
            // c = (uint8_t)base->FIFORD;
			// *(handle->ptrBuffer) = c;
			// rx_buffer_management(base, handle, c);
            g_callback_protocol_functions.rx_buffer_management(base);
		...
		if( (base->FIFOSTAT & USART_FIFOSTAT_TXNOTFULL_MASK)  != 0U ) {
			// c = *(handle->ptrBuffer);
			// buffer_management(base, handle, c);
			// if(SDK_USART_TX_STATUS__RUNNING == handle->Status) base->FIFOWR = c;
            g_callback_protocol_functions.tx_buffer_management(base);
    ...
    // if(SDK_USART_TX_STATUS__SEND_COMPLETED == handle->Status) {
 	if(g_callback_protocol_functions.tx_status_get__completed()) {
        // the tx procedure is going to end
        // the following procedure is usefull to confirm that the last byte is completely sent
        // it is important to manage rs485 enable/disable pins

        /* Tx idle and the interrupt is enabled. */
        if(
            ( 0U != (base->INTENSET & USART_INTENSET_TXIDLEEN_MASK) ) &&
            ( 0U != (base->INTSTAT  & USART_INTSTAT_TXIDLE_MASK)    )
        ) {
            // the last byte has just been completely sent
    ...
    // if(SDK_USART_RX_STATUS__READY_TO_READ == handle->Status) {
    if(g_callback_protocol_functions.rx_status_get__completed()) {
        // the last byte has just been received
        // note: if rxWatermark=0 the following procedure is unusefull

        /* restore if rx transfer ends and rxLevel is different from default value */
        if (USART_FIFOTRIG_RXLVL_GET(base) != handle->rxWatermark)
        {
            base->FIFOTRIG =
    ...
    // if(SDK_USART_RX_STATUS__RUNNING == handle->Status) {
	if(g_callback_protocol_functions.rx_status_get__running()) {
        // the rx is not ended yet

        // update rxWatermark
        // note: if rxWatermark=0 the following procedure is unusefull
        tmpsize = handle->PacketLength - handle->DataCounter;
...
void buffer_init(callback_protocol_functions_t* p_ptrCallbacks)  {
    g_ptr_callback_protocol_functions = p_ptrCallbacks;
}


void    USART_reg_write(void* p_ptrData, char p_data)	{
    ((USART_Type*) p_ptrData)->FIFOWR = p_data;
}

uint8_t USART_reg_read(void* p_ptrData)	{
    return ((USART_Type*) p_ptrData)->FIFORD;
}
...
// ******************************************************************************************
// ******************************************************************************************
void USART_RxTxCallback          (USART_Type *base, usart_handle_t *handle, status_t p_status, void *userData)  {

    switch (p_status) {
        case kStatus_USART_TxIdle:
            g_callback_protocol_functions.tx_status_set__txEnd();
            break;
        case kStatus_USART_RxIdle:
            g_callback_protocol_functions.rx_status_set__rxEnd();
            break;
    }
